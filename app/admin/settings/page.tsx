...
export default function AdminSettings() {
  ...
  const [pendingEmail, setPendingEmail] = useState('') // Store email that's pending confirmation
  ...
  useEffect(() => {
    ...
    // Listen for auth state changes (e.g., when email is confirmed)
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'USER_UPDATED' && session?.user?.email) {
        setCurrentEmail(session.user.email)
        setPendingEmail('') // Clear pending email
        setEmailSuccess(false) // Clear success message
      }
    })
    ...
    try {
      ...
      if (updateError) {
        ...
        if (updateError.message.includes('already registered') || updateError.message.includes('already exists')) {
          ...
          setEmailError('The email address format is invalid. Please check and try again.')
        } else {
      ...
      // Don't update current email state yet - wait for confirmation
      setPendingEmail(trimmedEmail) // Store the pending email for display
      setEmailSuccess(true)
      setEmailLoading(false)
      setNewEmail('')

      // Show success message for 10 seconds (longer since user needs to check email)
      setTimeout(() => {
        setEmailSuccess(false)
      }, 10000)
    } catch (err: any) {
