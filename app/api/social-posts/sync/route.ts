...
function parseCSVLine(line: string): SocialPostRecord | null {
  try {
    // The CSV contains JSON records, so parse the JSON
    const jsonStr = line.replace(/^"|"$/g, '').replace(/""/g, '"')
    const parsed = JSON.parse(jsonStr)
    
    // Clean and validate the data according to social_posts schema
    const cleanedData: SocialPostRecord = {
      publication: (parsed.publication || '').trim().substring(0, 255),
      image: parsed.image ? String(parsed.image).substring(0, 500) : null,
      url: parsed.url ? String(parsed.url).substring(0, 500) : null,
      platforms: Array.isArray(parsed.platforms) ? parsed.platforms : null,
      price: parsed.price ? String(parsed.price).substring(0, 50) : null,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const csvRecords: SocialPostRecord[] = []
...
export async function GET(request: Request) {
  return NextResponse.json({ 
    message: 'Use POST method to sync social posts data from CSV to database',
    endpoint: '/api/social-posts/sync'
  }, { 
    headers: {
      'Access-Control-Allow-Origin': '*',
      'Access-Control-Allow-Methods': 'POST, OPTIONS',
