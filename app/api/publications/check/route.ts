 ...
 * 
 * Accepts a list of publication names and checks if they exist in the database.
...
export async function POST(request: Request) {
  try {
    ...
    console.log(`🔍 [Publications Check API] Checking ${names.length} publication names...`)
    ...
    const existingNames = new Set(data?.map((pub: { name: string }) => pub.name) || [])
    ...
    console.log(`✅ [Publications Check API] Found ${existingCount} existing, ${missingCount} missing out of ${names.length} names`)
