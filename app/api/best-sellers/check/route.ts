...
/**
 * POST /api/best-sellers/check
 * 
 * Accepts a list of publication names and checks if they exist in the best_sellers table.
...
export async function POST(request: Request) {
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.error('❌ [Best Sellers Check API] Missing Supabase environment variables')
      return NextResponse.json(
    ...
    console.log(`🔍 [Best Sellers Check API] Checking ${names.length} publication names...`)
    
    const supabase = getSupabaseClient()

    // Query database for matching publication names in best_sellers table
    ...
    if (error) {
      console.error('❌ [Best Sellers Check API] Supabase query error:', error)
      return NextResponse.json(
    ...
    console.log(`✅ [Best Sellers Check API] Found ${existingCount} existing, ${missingCount} missing out of ${names.length} names`)

    return createFreshResponse(result)

  } catch (error: any) {
    console.error('❌ [Best Sellers Check API] Error:', error)
    return NextResponse.json(
