...
publications.forEach(pub => {
  if (pub.name && pub.niches && Array.isArray(pub.niches)) {
    // Get niche names from the niches array
    const nicheNames = pub.niches
      .map(n => n.name)
      .filter(Boolean);
    
    // Store with lowercase key for case-insensitive matching
    publicationNichesMap.set(pub.name.toLowerCase().trim(), nicheNames);
  }
});

console.log(`Found ${publicationNichesMap.size} unique publications with niches data in publicationsData`);
