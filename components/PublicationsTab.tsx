...
export default function PublicationsTab() {
  ...
  const getArticlePreviewUrl = (articlePreview: Publication['articlePreview']): string | null => {
    ...
    // Check for scraped originalUrl first (from browser scraper)
    const asset = articlePreview.asset as any
    if (asset._metadata && asset._metadata.originalUrl) {
      return asset._metadata.originalUrl
    }
    // Fall back to Sanity CDN URL construction
              ...
              <tbody className="divide-y divide-charcoal-700 relative z-0">
                {Array.isArray(filteredData) && filteredData.map((pub, index) => {
                  ...
                  <tr key={pub._id || index} className="text-sm">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          {pub.logo && pub.logo.asset ? (() => {
                            ...
                            if (asset._metadata && asset._metadata.originalUrl) {
                              // If originalUrl exists in metadata, use it directly
                              imageUrl = asset._metadata.originalUrl
                            } else if (asset._metadata && asset._metadata.isSupabaseUpload && asset._metadata.storagePath) {
                    ...
                    <td className="text-center text-gold-400 font-semibold">${price.toLocaleString()}</td>
                                  ...
                                  >
                                    ...
                                    >
                                      {niche.accepted && niche.price !== null ? (
                                        <span className="text-sm text-champagne">{niche.displayName} price: <span className="text-gold-400 font-semibold">${niche.price.toLocaleString()}</span></span>
                                      ) : (
