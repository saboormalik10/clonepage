...
export default function SocialPostTab() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            console.error('❌ [Social Posts] Authentication failed - redirecting to login')
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearchTerm(term)
    applyFilters(term, selectedGenres)
  }
  ...
  const toggleGenre = (genre: string) => {
    ...
    applyFilters(searchTerm, newGenres)
  }

  const applyFilters = (search: string, genres: string[]) => {
